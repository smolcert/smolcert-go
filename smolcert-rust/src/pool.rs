...
mod tests {
  ...
  fn test_cert_pool_validate_single_cert() {
    ...
    let root_cert = Certificate::new_self_signed(1, 
      "connctd".to_string(), 
      Validity::empty(), "connctd".to_string(), vec![], &root_keypair).unwrap();

    let root_certs = [root_cert];
    let cert_pool = CertificatePool::new(&root_certs[..]);

    let client_keypair = Keypair::generate(&mut csprng);
    let mut client_cert = Certificate::new(2, 
      "connctd".to_string(), Validity::empty(), 
      "client 1".to_string(), vec![], &client_keypair, &root_keypair).unwrap();
